                ...
                {filtered.length > 0 ? filtered.map(({ label, icon: Icon, href, color }) => (
                  <Link key={label} href={href}>
                    <div className="flex items-center gap-2 lg:gap-3 px-3 lg:px-4 py-3 rounded-[12px] border border-[rgba(0,0,0,0.08)] hover:border-[rgba(0,0,0,0.18)] hover:bg-[#fafafa] transition-colors cursor-pointer">
                      <div className="w-10 h-10 lg:w-14 lg:h-14 rounded-[10px] lg:rounded-[12px] flex items-center justify-center shrink-0" style={{ backgroundColor: `${color}18` }}>
                        <Icon size={22} strokeWidth={1.5} style={{ color }} />
                      </div>
                      <p className="text-[13px] lg:text-[14px] font-medium text-[#1a1a1a] tracking-[-0.04px] leading-tight">{label}</p>
                    </div>
                  </Link>
                )) : (
                  <p className="text-[14px] text-[#7c7c7c] col-span-2 lg:col-span-3">Ештеңе табылмады</p>
                )}
              </div>
            </div>
